...
export class Engine {
  ...
  _ensureWorker() {
    ...
    this.worker.onerror = (err) => {
      ...
      for (const [, pending] of this._pending) pending.reject(new Error(message));
      this._pending.clear();
    };
  }
  ...
  analyze({ id, name, bytes, done, total }) {
    if (!this.ready) return Promise.reject(new Error("Engine is not ready"));
    const jobId = id || "job-" + Math.random().toString(16).slice(2);
    const copy =
      bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes instanceof ArrayBuffer ? bytes : []);
    return new Promise((resolve, reject) => {
      this._pending.set(jobId, { resolve, reject });
      this.worker.postMessage({ type: "analyze", id: jobId, name, bytes: copy, done, total });
    });
  }
  ...
  destroy() {
    ...
    this.ready = false;
    this._status = "idle";
    for (const [, pending] of this._pending) pending.reject(new Error("Engine restarted"));
    this._pending.clear();
  }
}
